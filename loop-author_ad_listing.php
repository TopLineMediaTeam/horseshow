...
if($term->slug)
	$args = array( 'post_type' => 'ad_listing','ad_cat' =>$term->slug, 'author' => $author, 'posts_per_page' =>2,'paged' => $paged);
else
	$args = array( 'post_type' => 'ad_listing', 'author' => $author,'posts_per_page' =>5,'paged' => $paged,'meta_query' =>array('relation' => 'AND', array('key' => 'cp_type','value' => 'buyer')));	
...
$flag=1;
 ...
 $sql="SELECT meta_value FROM wp_postmeta WHERE meta_key='cp_type' AND post_id=".$meta_id;
      $post_details = $wpdb->get_results($sql);
	//  echo  $sql;
//	  echo $post_details[0]->meta_value.'<br>';
	  if($post_details[0]->meta_value=='buyer')
	  {
	  	//continue;
