				...
				<?php
					...
					switch ( $result->field_type ) {
						...
						case 'text box':
							...
							{
								echo  ('<label class="form-sub-label" for="first_3" id="sublabel_first" style="color:#fff;">Price </label><br>');
							}
					...
					<?php
						...
						case 'checkbox':
							...
							$html_checkbox = '';
							$html_options = '';
							$html_options .= html( 'li', array(), $result->field_label );
							foreach ( $options as $option ) {
...
// loops through the custom fields and builds the custom ad form for buyer
if ( !function_exists('cp_formbuilder_buyer') ) {
	...
	?>
			...
			<li id="list_<?php echo esc_attr($result->field_name); ?>" <?php /*if($result->field_name=='post_title') echo 'style="display:none;"'; */?>>
				<?php /*?><div class="labelwrapper">
					<label><?php if ( $result->field_tooltip ) { ?><a href="#" tip="<?php echo esc_attr( translate( $result->field_tooltip, APP_TD ) ); ?>" tabindex="999"><div class="helpico"></div></a><?php } ?><?php echo esc_html( translate( $result->field_label, APP_TD ) ); ?>: <?php if ( $result->field_req ) echo '<span class="colour">*</span>'; ?></label>
					<?php if ( ($result->field_type) == 'text area' && ( $cp_options->allow_html ) ) { // only show this for tinymce since it's hard to position the error otherwise ?>
						<br /><label class="invalid tinymce" for="<?php echo esc_attr($result->field_name); ?>"><?php _e( 'This field is required.', APP_TD ); ?></label>
				...
				<?php
					if($result->field_name=='cp_video_url')//hide video url form buyer form
						{
							continue;
						}
					switch ( $result->field_type ) {
...
function cp_other_fields($renew_id = false) {
	...
	if ( cp_payments_is_enabled() ) {
		...
		?>
			...
			<?php /*?><li id="list_featured_ad" class="withborder">
				<div class="labelwrapper">
...
if ( !function_exists('cp_show_form') ) {
	function cp_show_form($catid, $renew = false,$type='seller') {
		global $wpdb, $cp_options;
...
//queries the db for the custom ad form based on the cat id and buyer
if ( !function_exists('cp_show_form_buyer') ) {
