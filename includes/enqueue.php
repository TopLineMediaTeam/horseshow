...
function cp_load_scripts() {
	...
	if ( is_singular() ) {
		...
		if ( $cp_options->form_val_lang ) {
			...
			wp_enqueue_script( 'validate-lang', APP_FRAMEWORK_URI . "/js/validate/i18n/messages_$lang_code.js", array( 'validate' ) );
		}
	}

	//added for modal popup
    wp_enqueue_script( 'modal', get_template_directory_uri() . '/includes/js/modal.js', array( 'jquery' ), '1.1' );
	...
	//if ( is_page_template( 'tpl-profile.php' ) )
		//wp_enqueue_script( 'user-profile' );// blocked by rj to avoid the  password text issues in my profile
