...
add_action( 'wp_ajax_removehorses_from_show_function', 'removehorses_from_show_function' );
add_action("wp_ajax_nopriv_removehorses_from_show_function", "removehorses_from_show_function");
...
if (!function_exists('cp_get_image_url')) {
	function cp_get_image_url() {
		...
		// no image so return the placeholder thumbnail
			...
			$authorimage= get_avatar( $post->post_author,300 );
			echo $authorimage;
...
function removehorses_from_show_function(){
	...
	if( isset($_GET['action']) && $_GET['action'] == 'removehorses_from_show_function'){
		...
		$userid = $_GET['uid'] ;
		$showid = $_GET['sid'] ;
		$result = $wpdb->get_results("DELETE FROM horseshows_meta WHERE post_id='".$postid."' and user_id='".$userid."' and show_id='".$showid."'");
		
		}
	echo "DELETE FROM horseshows_meta WHERE post_id='".$postid."' and user_id='".$userid."' and show_id='".$showid."'";
	die();
}
//created by rj to avoid selected menu in the case of search.	
function av_custom_menu()
{	
