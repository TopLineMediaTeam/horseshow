	...
	<li>
		<div id="app-attachment-upload-container">
			...
			<div class="app-attachment-info">
				...
				<p class="small"><?php printf("Please upload minimum 720px wide images."); ?><br /><?php printf( __( 'You are allowed to upload %s file(s).', APP_TD ), $options['allowed_files'] ); ?> <?php printf( __( 'Maximum file size: %s KB.', APP_TD ), $options['max_file_size'] ); ?><br /><?php printf("*Do not post photographers proofs or photos you have not purchased."); ?></p>
				<?php if ( ! isset( $options['disable_switch'] ) || ! $options['disable_switch'] ) { ?>
					<p class="small upload-flash-bypass"></p>
					<p class="small upload-html-bypass"></p>
				<?php } ?>
			</div>
		</div>
		<div class="clear"></div>
